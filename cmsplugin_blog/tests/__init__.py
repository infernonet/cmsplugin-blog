...
class BlogTestCase(BaseBlogTestCase):
    ...
    def test_01_apphook_added(self):
        ...
        self.assertEquals(urls.reverse('blog_archive_index'), '/en/test-page-1/')
        translation.activate('de')
        self.assertEquals(urls.reverse('blog_archive_index'), '/de/test-page-1/')
    ...
    def test_03_admin_add(self):
        ...
        add_url = urls.reverse('admin:cmsplugin_blog_entry_add')
    ...
    def test_04_admin_change(self):
        ...
        edit_url = urls.reverse('admin:cmsplugin_blog_entry_change', args=(str(entry.pk)))
    ...
    def test_05_admin_add_post(self):
        ...
        edit_url = urls.reverse('admin:cmsplugin_blog_entry_change', args=(1,))
    ...
    def test_06_admin_changelist(self):
        ...
        changelist_url = urls.reverse('admin:cmsplugin_blog_entry_changelist')
...
class BlogRSSTestCase(BaseBlogTestCase):
    ...
    def test_01_posts_one_language(self):
        ...
        title, entry = self.create_entry_with_title(published=True,
            published_at=published_at)
        response = self.client.get(urls.reverse('blog_rss'))
        self.assertEquals(response.status_code, 200)
    ...
    def test_02_posts_all_languages(self):
        ...
        title, entry = self.create_entry_with_title(published=True,
            published_at=published_at)
        response = self.client.get(urls.reverse('blog_rss_any'))
        self.assertEquals(response.status_code, 200)
    ...
    def test_03_posts_by_author_single_language(self):
        ...
        response = self.client.get(urls.reverse('blog_rss_author', kwargs={'author': user.username}))
        self.assertEquals(response.status_code, 200)
        self.assertContains(response, 'in English')
    ...
    def test_04_posts_by_author_all_languages(self):
        ...
        title, entry = self.create_entry_with_title(published=True,
            published_at=published_at, author=user)
        response = self.client.get(urls.reverse('blog_rss_any_author', kwargs={'author': user.username}))
        self.assertEquals(response.status_code, 200)
        self.assertNotContains(response, 'in English')
    ...
    def test_05_posts_tagged_single_language(self):
        ...
        title, entry = self.create_entry_with_title(published=True,
            published_at=published_at)
        response = self.client.get(urls.reverse('blog_rss_tagged', kwargs={'tag': 'test'}))
        self.assertEquals(response.status_code, 200)
        self.assertContains(response, 'in English')
    ...
    def test_06_posts_tagged_all_languages(self):
        ...
        title, entry = self.create_entry_with_title(published=True,
            published_at=published_at)
        response = self.client.get(urls.reverse('blog_rss_any_tagged', kwargs={'tag': 'test'}))
        self.assertEquals(response.status_code, 200)
        self.assertNotContains(response, 'in English')

    def test_07_no_multilingual(self):
        mwc = [mw for mw in settings.MIDDLEWARE if mw != 'cmsplugin_blog.middleware.MultilingualBlogEntriesMiddleware']
        with SettingsOverride(MIDDLEWARE=mwc):
            published_at = datetime.datetime.now() - datetime.timedelta(hours=1)
            title, entry = self.create_entry_with_title(published=True,
                published_at=published_at)
            response = self.client.get(urls.reverse('blog_rss'))
...
class ViewsTestCase(BaseBlogTestCase):
    ...
    def test_01_generics(self):
        ...
        response = self.client.get(urls.reverse('blog_archive_index'))
        self.assertEquals(response.status_code, 200)

        response = self.client.get(urls.reverse('blog_archive_year', kwargs={'year': published_at.strftime('%Y')}))
        self.assertEquals(response.status_code, 200)

        response = self.client.get(urls.reverse('blog_archive_month',
            kwargs={
        ...
        response = self.client.get(urls.reverse('blog_archive_day',
            kwargs={
                'year': published_at.strftime('%Y'),
                'month': published_at.strftime('%m'),
        ...
        response = self.client.get(urls.reverse('blog_detail',
            kwargs={
                'year': published_at.strftime('%Y'),
                'month': published_at.strftime('%m'),
                'day': published_at.strftime('%d'),
        ...
        response = self.client.get(urls.reverse('blog_archive_tagged',
            kwargs={
                'tag': 'test'
            }))
        self.assertEquals(response.status_code, 200)

        response = self.client.get(urls.reverse('blog_archive_author',
            kwargs={
                'slug': user.username
            }))
        self.assertEquals(response.status_code, 200)
...
class TimeZoneTestCase(BaseBlogTestCase):
    ...
    def test_01_default_timezone_returns_proper_day(self):
        ...
        with SettingsOverride(USE_TZ = True, TIME_ZONE = "Canada/Eastern"):
            ...
            response = self.client.get(urls.reverse('en:blog_archive_day',
                kwargs={
